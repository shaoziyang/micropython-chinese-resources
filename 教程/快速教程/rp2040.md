...
wlan = network.WLAN()       # 创建默认接口
wlan.active(True)           # 激活接口
wlan.scan()                 # 扫描接入点
wlan.isconnected()          # 检查是否连接到 AP
wlan.connect('ssid', 'key') # 连接到 AP
wlan.config('mac')          # 获取 MAC 地址
wlan.ipconfig('addr4')      # 获取 IPv4 地址

ap = network.WLAN(network.WLAN.IF_AP) # 创建 AP 接口
ap.config(ssid='RP2-AP')    # 设置 SSID 名称
ap.config(max_clients=10)   # 设置最大连接数
ap.active(True)             # 激活接口
```
